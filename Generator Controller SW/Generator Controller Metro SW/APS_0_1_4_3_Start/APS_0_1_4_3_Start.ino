...
/*
  Autonomous Power Solutions Rev 0.1.4.3
  Generator Powered Battery Charger
  
  
   Rev 0.1.4.2 is designed to run on the S/W development prototype PCA
  ...
  FAN TFEATURE WILL BE REMOVED
  FAN OPERATION
  Battery temperature of 50F to 75F is safe range. No action required.
 ...
 *----------------------------------------------------------- */                  
 ...
 const int fanControl = 6;  //FAN FEATURE WILL BE REMOVED
 const int faultAlarm = 9;
 const int startEnable = 7;
 const int startCommand = 10;
 const int loadControl = 8;
 const int q1 = 11;
 const int q2 = 12;
 const int q4 = 13;
 ...
 unsigned long countMod;     //used to hold modulus of countHour
 ...
 boolean fanOn = false;      //fan signal is ON  FAN FEATURE WILL BE REMOVED
 ...
 //SYSTEM BATTERY VOLTAGE MEASUREMENT FUNCTION
  ...
  return battVolt;
}//Closing brace for readBattVolt() function
 ...
 //SYSTEM BATTERY CHARGE CURRENT MEASUREMENT FUNCTION
  ...
  battCurrent=0.0; //Initialize battCurrent
   ...
   return battCurrent;
}//Closing brace for readBattAmps() function
...
} //Closing brace for readBattTemp() function
...
//BATTERY ENCLOSURE AMBIENT TEMPERATURE MEASUREMENT FUNCTION
 ...
 return ambTemperature;
   ...
   void checkFault() {
   if (digitalRead(oilSwitch) == LOW){ //DETERMINE IF OIL LEVEL FAULT EXISTS
       oilFault = true;
       }
      else {
        oilFault = false;
      ...
      battTemperature = readBattTemp(); //DETERMINE IF A BATTERY TEMPERATURE FAULT EXISTS
   if (battTemperature >=100.0) {
       ...
       battVolt = readBattVolt(); //DETERMINE IF A BATTERY VOLTAGE FAULT EXISTS
   if (battVolt <=22.0) {
      voltFault = true;
      }
      else {
      ...
      battCurrent = readBattAmps(); //DETERMINE IF A CURRENT FAULT EXISTS
      if ((engineRun==true) && ((battCurrent >90.0) || (battCurrent <30.0))){
          currentFault = true;
          }
      else { 
        currentFault = false;
   ...
   if ((oilFault == false) && (tempFault == false) && (voltFault == false) && (currentFault == false)){
   //set up lcd and and send welcome message THIS WILL CAHNGE AS MAN/AUTO MODE CODE STARTS WORKING
    digitalWrite(faultAlarm,LOW);  //clear Fault Alarm contact closure
    lcd.clear();
    delay(100);
    lcd.setCursor(0,0);
...
//Check fan function
      void checkFan(){ //READ TEMPERTURE AND TURN ON FAN IF REQUIRED
      battTemperature = readBattTemp();
      ambTemperature = readAmbTemp();
       Serial.print("Fan Control= ");
       Serial.println(fanOn);
      if (((fanOn == true) && (battTemperature<80.0)) || (battTemperature-ambTemperature <5.0)){
        ...
        fanOn = false; 
          ...
          } //Closing brace for checkFan() function
...
int readSwitch() {
 ...
 return runTime;
} //Closing brace for readSwitch() function
...
void switchPressed1()
{
...
} // closing brace for switchedPressed() ISR
 
 // INSERT TIMER 1 INTERRUPT ROUTINE HERE OR IN SETUP WHERE THE TIMER IS CONFIGURED?
  ...
  */
  ...
  void setup() {
   ...
   modeTotal=0;  //This is run one time in setup(), otherwise it happens in ISR 
   if (digitalRead(manual)==HIGH) { modeTotal+=1; }
   if (digitalRead(automatic)== HIGH) { modeTotal+=2; }
   switch (modeTotal) {
    case 0:
    ...
    digitalWrite(startEnable,HIGH);     //turn on for test purpose
    engineRun=true;                     //turn on for test purpose
      
    checkFault(); //DETERMINE IF ANY FAULTS EXISTS
    checkFan(); //CHECK TO SEE IF FAN SHOULD BE ON
 ...
 void loop() {
  ...
  //Check for a Fault Condition
   Serial.println("Check for faults");
   checkFault(); //DETERMINE IF ANY FAULTS EXISTS
   Serial.println("Check fan cooling");
   checkFan(); //CHECK TO SEE IF FAN SHOULD BE ON
   ...
   readSensor(A3); //Check crank battery voltage
   ...
   Serial.println(mode); 
   runTime = readSwitch();
   Serial.print("engineRun=");
   Serial.println(engineRun);
   Serial.println("------------------------- ");
   delay(3000);
